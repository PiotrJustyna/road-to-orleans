...
docker build -t silo-host -f ./ops/SiloHost/Dockerfile ./ &&
  docker run -it -e AWS_SECRET_ACCESS_KEY -e AWS_SESSION_TOKEN -e AWS_ACCESS_KEY_ID -e ADVERTISEDIP=$ADVERTISEDIP -e GATEWAYPORT=$GATEWAYPORT -p $GATEWAYPORT:3000 -p 8080:8080 --rm silo-host